...
> **For AI coding agents:** Use the [`docs/`](docs/README.md) version instead – same content split into smaller files so agents can load only what they need.
...
### Critical: always use a fresh bridge reference

In ehpk mode, a bridge reference captured once during initialisation can go stale. Writes via a stale reference silently fail – the promise resolves but nothing persists. This does not happen in the dev simulator or QR prototype mode, making it hard to catch during development.

**Do not** store the bridge in a module-level variable and reuse it for writes:

```typescript
// BAD – works in dev, silently breaks in ehpk
let bridge: EvenAppBridge | null = null
export function init(b: EvenAppBridge) { bridge = b }
export function save(k: string, v: string) {
  if (bridge) bridge.setLocalStorage(k, v)  // stale reference in ehpk
}
```

**Do** call `getBridge()` fresh on every read/write, matching the weather app pattern:

```typescript
// GOOD – always current
export async function save(k: string, v: string) {
  const b = getBridge()  // fresh reference from state.ts
  if (b) await b.setLocalStorage(k, v)
}
```

### Always await storage writes

Fire-and-forget (`void bridge.setLocalStorage(...)`) can lose data over the bluetooth relay. Always `await` the call so the phone confirms persistence before the caller continues.

### Browser localStorage does not survive ehpk restarts

The ehpk WebView has no stable origin, so `localStorage` is wiped between sessions. Only `bridge.setLocalStorage` / `bridge.getLocalStorage` persists across app restarts on real hardware. In dev/QR mode `localStorage` happens to work (stable HTTP origin), which masks the issue.

### Static assets in ehpk

Absolute paths like `/catalog.json` do not resolve in the ehpk WebView (no HTTP origin). Use Vite's `?url` asset import to get a build-time resolved path:

```typescript
import catalogUrl from './catalog-data.json?url'
const res = await fetch(catalogUrl)  // works in ehpk, Vercel, and dev
```
