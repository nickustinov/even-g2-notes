...
| `width` | number | 0–576 | SDK documents 20–200 for images, but larger sizes work |
| `height` | number | 0–288 | SDK documents 20–100 for images, but larger sizes work |
...
| Constraint | Value |
|---|---|
| Width | SDK documents 20–200 px, but larger sizes (up to 576) work in practice |
| Height | SDK documents 20–100 px, but larger sizes (up to 288) work in practice |
...
When your primary display is a full-screen image (Canvas-rendered UI, games, etc.), you need a separate container with `isEventCapture: 1` to receive input events since image containers don't have this property.
...
Instead, place a full-screen text container (content: `' '`) **behind** the image. The text container receives all event types including scroll. The image container renders on top (higher container ID = drawn later), so the text is invisible:
...
This gives you click, double-click, scroll-top and scroll-bottom events while the image is fully visible. Events arrive as `textEvent` (not `listEvent`), so adjust your event handler accordingly.
...
Use `@evenrealities/evenhub-cli` to package your app for distribution:

```bash
npm install -D @evenrealities/evenhub-cli
```

Add an `app.json` at your project root:
...
Common scripts (see [reddit](https://github.com/fuutott/rdt-even-g2-rddit-client) for a working example):

```json
{
  "scripts": {
    "build": "vite build",
    "pack": "evenhub-cli pack",
    "pack:check": "evenhub-cli pack --check",
    "qr": "evenhub-cli qr"
  }
}
```

`evenhub-cli pack` builds a distributable package from `vite build` output. `evenhub-cli qr` generates a QR code for sideloading onto the Even App.
